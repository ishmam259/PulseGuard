...
export default function Users() {
  ...
  return (
    <AdminLayout title={$('ADMIN_TITLE_USERS', locale)}>
      {/* Header Actions */}
      <div className="card-row animate-fade-in" style={{ marginTop: 0, marginBottom: 'var(--spacing-4)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <input
          className="input"
          placeholder={$('ADMIN_USERS_SEARCH', locale)}
          aria-label={$('ADMIN_USERS_SEARCH', locale)}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          style={{ maxWidth: '400px' }}
        />
        <button className="btn btn--primary" type="button" onClick={() => setShowModal(true)}>
          {$('ADMIN_USERS_ADD', locale)}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="animate-fade-in" style={{ animationDelay: '100ms' }}>
          <table className="table">
            <thead>
              <tr>
                <th>{$('ADMIN_USERS_COL_ID', locale)}</th>
                <th>{$('ADMIN_USERS_COL_NAME', locale)}</th>
                <th>{$('ADMIN_USERS_COL_ROLE', locale)}</th>
                <th>{$('ADMIN_USERS_COL_EMAIL', locale)}</th>
                <th>{$('ADMIN_USERS_COL_PHONE', locale)}</th>
                <th>{$('ADMIN_USERS_COL_CREATED', locale)}</th>
      ...
      {!loading && (
        <div className="card-row" style={{ marginTop: 'var(--spacing-4)' }}>
          <p className="muted">{$('ADMIN_USERS_SHOWING', locale)} {filtered.length} {$('ADMIN_USERS_OF', locale)} {users.length} users</p>
          <div className="chip-row">
            <span className="chip" style={{ background: 'rgba(16,185,129,0.1)', color: '#10b981' }}>
              {$('ADMIN_USERS_WORKERS', locale)} {users.filter((u) => u.role === 'worker').length}
            </span>
            <span className="chip" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366f1' }}>
              {$('ADMIN_USERS_ADMINS', locale)} {users.filter((u) => u.role === 'admin').length}
            </span>
            <span className="chip" style={{ background: 'rgba(245,158,11,0.1)', color: '#f59e0b' }}>
              {$('ADMIN_USERS_PATIENTS', locale)} {users.filter((u) => u.role === 'patient').length}
            </span>
          </div>
        </div>
      )}
      ...
      {showModal && (
        <div className="modal-backdrop">
          <div className="card animate-fade-in" style={{ width: '100%', maxWidth: '500px', padding: '2rem' }}>
            <h3>{$('ADMIN_USERS_MODAL_TITLE', locale)}</h3>
            {error && (
